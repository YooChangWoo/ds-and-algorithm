...
# 전역변수
root = None
groupList = ['블랙핑크', '레드벨벳', '마무무', '에이핑크', ' 걸스데이', '트와이스']
...
for name in groupList[1:]: # 레드벨벳붜 끝까지
    node = TreeNode()
    node.data = name
...
while True:
    if deletName == curr.data: # 삭제할 데이터 찾았으면
        if curr.left == None and curr.right == None: # 리프노드라 삭제 무지 쉬움
            if parent.left == curr: # 내가 부모의 왼쪾에 붙어있으면
                parent.left = None
            else: # 내가 부모의 오른쪾에 붙어있으면
                parent.right = None
